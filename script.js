// Wait until the DOM is fully loaded (optional safety)
document.addEventListener('DOMContentLoaded', () => {
  const addEventBtn = document.getElementById('add-event');
  const generateBtn = document.getElementById('generate-ics');
  const container = document.getElementById('events-container');

  // Format Date object to local floating time (YYYYMMDDTHHMMSS)
  const formatLocal = (d) => {
  ...
  generateBtn.addEventListener('click', () => {
    ...
    eventBlocks.forEach((block, index) => {
      ...
      if (date && time) {
        ...
        const endDate = new Date(startDate.getTime() + duration * 60000); // add minutes in ms

        events.push({
          uid: Date.now() + index + "@mycalendar",
    ...
    if (events.length === 0) {
      alert("Please fill at least one event with date and time.");
      return;
    }
...
VERSION:2.0
PRODID:-//Nirajan Rijal//Multi-Event ICS Local//EN
`;
